...
/**
 * Normalizes module-style generated screens into the runtime contract expected by the iframe shell.
 * This keeps older/demo payloads (import/export default) from crashing preview init.
 */
function normalizeComponentCode(code: string): string {
  ...
  normalized = normalized.replace(
    /^\s*export\s+default\s+function\s+([A-Za-z0-9_$]+)\s*\(/m,
    'window.__SCREEN_COMPONENT__ = function $1('
  );
  normalized = normalized.replace(
    /^\s*export\s+default\s+function\s*\(/m,
    'window.__SCREEN_COMPONENT__ = function('
  );
  normalized = normalized.replace(
    /^\s*export\s+default\s+/m,
    'window.__SCREEN_COMPONENT__ = '
  );
  normalized = normalized.replace(/^\s*export\s+\{[^}]*\};?\s*$/gm, '');
  return normalized;
}
...
(function() {
  ...
  function normalizeComponentCode(code) {
    var normalized = code;
    normalized = normalized.replace(/^\\s*import(?:[\\s\\S]*?)from\\s+['"][^'"]+['"];\\s*$/gm, '');
    normalized = normalized.replace(
      /^\\s*export\\s+default\\s+function\\s+([A-Za-z0-9_$]+)\\s*\\(/m,
      'window.__SCREEN_COMPONENT__ = function $1('
    );
  ...
  function loadComponent(code, initialState) {
    ...
    function RuntimeBridge() {
      ...
      function onRuntimeAction(action) {
        var requestId = 'rt_' + Date.now() + '_' + Math.random().toString(36).slice(2);
        return new Promise(function(resolve, reject) {
          function handler(e) {
            if (!e.data || e.data.type !== 'runtimeActionResult' || e.data.requestId !== requestId) return;
            window.removeEventListener('message', handler);
            if (e.data.error) {
              reject(new Error(e.data.error));
              return;
            }
            var patch = e.data.statePatch || {};
            setAppState(function(prev) { return Object.assign({}, prev, patch); });
            resolve(patch);
          }
          window.addEventListener('message', handler);
          window.parent.postMessage({ type: 'runtimeAction', requestId: requestId, action: action, state: appState }, '*');
        });
      }

      try {
        return React.createElement(Comp, { state: appState, onNavigate: onNavigate, onStateChange: onStateChange, onRuntimeAction: onRuntimeAction });
      } catch(e) {
...
(function() {
  ...
  var componentCode = normalizeComponentCode(\`${escapedCode}\`);
  var transpiled;
  ${needsBabel ? `
  if (typeof Babel === 'undefined') {
    showError('Babel failed to load. Check your internet connection and refresh.');
